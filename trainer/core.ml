...
type t = {
  name : string;
  creature1 : Creature.Core.t;
  creature2 : Creature.Core.t;
  creature3 : Creature.Core.t;
  revive_used : bool;
}
...
let all_alive trainer =
  Creature.Core.dead trainer.creature1 |> not
  && Creature.Core.dead trainer.creature2 |> not
  && Creature.Core.dead trainer.creature3 |> not

let all_dead trainer =
  Creature.Core.dead trainer.creature1
  && Creature.Core.dead trainer.creature2
  && Creature.Core.dead trainer.creature3
...
let use_move trainer move =
  { trainer with creature1 = Creature.Core.use_move_with_name trainer.creature1 move }
...
let switch trainer creature_name =
  ...
  let switched_out = Creature.Core.reset trainer.creature1 true in
  match order with
...
let revive (trainer : t) revived_name =
  ...
  | false ->
      ...
      if all_alive trainer then raise NoCreaturesDead
        ...
        if Creature.Core.dead trainer.creature1 then
          {
            trainer with
            creature1 =
              {
                (Creature.Core.reset trainer.creature1 true) with
        ...
        if Creature.Core.dead trainer.creature2 then
          {
            trainer with
            creature2 =
              {
                (Creature.Core.reset trainer.creature2 true) with
        ...
        if Creature.Core.dead trainer.creature3 then
          {
            trainer with
            creature3 =
              {
                (Creature.Core.reset trainer.creature3 true) with
...
let trainer_string trainer =
  Printf.sprintf "%s%s%s%s%s%s" trainer.name
    ...
    ("\n" ^ Creature.Core.as_string trainer.creature1 ^ " (IN BATTLE)")
    (if Creature.Core.dead trainer.creature2 && trainer.revive_used then ""
    else "\n" ^ Creature.Core.as_string trainer.creature2)
    (if Creature.Core.dead trainer.creature3 && trainer.revive_used then ""
    else "\n" ^ Creature.Core.as_string trainer.creature3)
    ("\n" ^ Creature.Core.moves_as_string trainer.creature1)
